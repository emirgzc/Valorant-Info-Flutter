...
import 'package:valoinfos/feature/char/sound.dart';
...
class _CharDetailPageState extends State<CharDetailPage> {
  ...
  Widget _abilitiesDesc(DataCharOne? datas, String color) {
    return Text(
      datas?.abilities?[indexAbi].description ?? StringConstants.noData,
  ...
  Widget _abilitiesName(DataCharOne? datas, String color) {
    return Text(
      datas?.abilities?[indexAbi].displayName?.toUpperCase() ?? StringConstants.noData,
  ...
  Widget _descChar(DataCharOne? datas, String colorThree) {
    return Text(
      datas?.description ?? StringConstants.noData,
      style: context.theme.bodyMedium!.copyWith(
        color: Color(
          int.parse(colorThree),
        ),
  ...
  Widget _roleChar(DataCharOne? datas, String colorThree) {
    return Padding(
      ...
      child: Text(
        datas?.role?.displayName?.toUpperCase() ?? StringConstants.noData,
  ...
  Widget _charName(DataCharOne? datas, String colorTwo) {
    return RotatedBox(
      ...
      child: BorderedText(
        ...
        child: Text(
          datas?.displayName?.toUpperCase() ?? StringConstants.noData,
          style: TextStyle(
            letterSpacing: 4,
  ...
  Widget _backgroundImage(DataCharOne? datas, String colorTwo) {
    return CacheImage(
      image: datas?.background ?? StringConstants.noData,
      fit: BoxFit.contain,
      height: 800.h,
      color: Color(int.parse(colorTwo)),
    );
